...
\paragraph*{Base Path}
Point this at the folder that contains your ``GameData'' tree (e.g., ``C:/MyGame/GameData/'') so the importer
can prepend it to texture paths and the linked .gla when your working files live somewhere else.

\paragraph*{.gla override}
Supply another GLA path in Jedi Academy base-relative notation (for example ``models/players/\_humanoid/\_humanoid.gla'')
to make the GLM load a different skeleton, as long as the bone count matches.
...
normally not children of the upper arms) into something easier to animate while keeping exports working.
...
\paragraph*{Base Path}
Give the folder that contains your ``GameData'' tree (for example ``C:/MyGame/GameData/'') so the exporter
can write the correct relative texture and skeleton paths into the GLM.
...
\paragraph*{Base Path}
Point to the folder holding ``GameData'' (e.g., ``C:/MyGame/GameData/'') so the exporter records the right
relative paths when writing your GLA.
